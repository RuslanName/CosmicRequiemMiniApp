...
export class ClanController {
  ...
  @ApiResponse({
    ...
    description:
      'Рейтинг кланов, отсортированных по силе и деньгам (от самых крутых к менее крутых)',
  })
  @ApiResponse({ status: 401, description: 'Не авторизован' })
  async getClanRating(
    @Query() paginationDto?: PaginationDto,
  ...
  @ApiOperation({
    ...
    description:
      'Создает новый клан из сообщества VK. Требуется ID сообщества (vk_group_id). Данные группы (название и изображение) получаются автоматически через VK API с использованием сервисного токена. В противном случае можно указать name и image_url вручную.',
  })
