...
export class ClanService {
  ...
  async declareWar(userId: number, targetClanId: number): Promise<ClanWar> {
    ...
    const activeWarsCount = await this.clanWarRepository.count({
      where: [
        {
          clan_1: { id: targetClanId },
          status: ClanWarStatus.IN_PROGRESS,
        },
        {
          clan_2: { id: targetClanId },
          status: ClanWarStatus.IN_PROGRESS,
        },
      ],
    });
  ...
  async getEnemyClanMembers(userId: number): Promise<User[]> {
    ...
    const activeWars = await this.clanWarRepository.find({
      where: [
        { clan_1: { id: user.clan.id }, status: ClanWarStatus.IN_PROGRESS },
        { clan_2: { id: user.clan.id }, status: ClanWarStatus.IN_PROGRESS },
      ],
    ...
    return await this.userRepository.find({
      where: enemyClanIds.map(clanId => ({clan: {id: clanId}})),
  ...
  async attackEnemy(userId: number, targetUserId: number): Promise<{
    ...
    const activeWar = await this.clanWarRepository.findOne({
      where: [
        { clan_1: { id: attacker.clan.id }, clan_2: { id: defender.clan.id }, status: ClanWarStatus.IN_PROGRESS },
        { clan_1: { id: defender.clan.id }, clan_2: { id: attacker.clan.id }, status: ClanWarStatus.IN_PROGRESS },
      ],
    });
